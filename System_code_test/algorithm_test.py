...
def link_node(node1, node2):  # 잘못연결하는 것을 방지하기 위해 좌표상으로 x,y,z축 중 1만큼 차이나는 것만 연결
    if (node1.location[0] + 1 == node2.location[0] and node1.location[1:2] == node2.location[1:2]):
        ...
        node1.direction[2] = 1
        node2.direction[3] = 1
    elif (node1.location[1] + 1 == node2.location[1] and node1.location[0] == node2.location[0] and node1.location[2] ==
          node2.location[2]):
        node1.forward = node2
...
if __name__ == "__main__":
    ...
    node.append(AddNode(5, 3, 3)), node.append(AddNode(6, 3, 3)), node.append(
        AddNode(7, 3, 3))  # 구름다리 4층 Node_num = 49 ~ 51
    node.append(AddNode(5, 3, 4)), node.append(AddNode(6, 3, 4)), node.append(
        AddNode(7, 3, 4))  # 구름다리 5층 Node_num = 52 ~ 54
    ...
    link_node(node[13], node[0]), link_node(node[1], node[15]), link_node(node[7], node[21]), link_node(node[11],
                                                                                                        node[25])

    # 2호관 2층 연결
    for i in range(14, 27):
        link_node(node[i], node[i + 1])
    link_node(node[27], node[14]), link_node(node[15], node[35]), link_node(node[21], node[41]), link_node(node[25],
                                                                                                           node[45])
    ...
    # 구름다리 연결
    link_node(node[21], node[48]), link_node(node[48], node[49]), link_node(node[49], node[50]), link_node(node[50],
                                                                                                           node[69]),
    link_node(node[41], node[51]), link_node(node[51], node[52]), link_node(node[52], node[53]), link_node(node[53],
                                                                                                           node[74]),
    ...
    print('20->', node[33].exit_diret)
    fire_test(31, 31, 31, node, linked_node_list)
    print('20->', node[33].exit_diret)
